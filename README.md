...
- **不要写自定义 Godot Node 子类去覆盖 `_Ready()`/`_Process()`**：曾经为了修复晶壤图标"开局不显示"的时序问题，把图标行改写成一个自定义 `PipRow : HBoxContainer` 子类，结果导致每次进入战斗时 MonoMod 的 JIT 钩子直接抛异常（`InvokeGodotClassMethod` 内部报错），异常一路冒泡到 `NRun.SetCurrentRoom`，导致整个战斗房间搭建流程中断——不仅晶壤图标，连角色立绘和血条都因此消失了。根本原因未完全查清（疑似这个魔改环境下 MonoMod 热补丁和自定义节点子类的虚方法覆盖有冲突），但结论很明确：战斗内UI节点只用普通的内置节点类型（HBoxContainer/TextureRect/Control）拼装，不要自己继承出新的Node子类。时序类问题改用 `CallDeferred(...)` 来解决，不要用自定义子类的每帧轮询。
